...
export default function UserListPage() {
  ...
  const fetchUsers = async () => {
    ...
    if (filters.upazila) queryParams.branchName = filters.upazila;
    if (filters.status) queryParams.status = filters.status;
    if (filters.branchCode) queryParams.branchCode = filters.branchCode;

    try {
      const res = await fetch(`/api/users/admin?${new URLSearchParams(queryParams)}`);
  ...
  // Fetch users on component mount or when filters/page change
  useEffect(() => {
    fetchUsers();
  }, [page, filters]);
  ...
  const handleApproveUser = async (userId: string) => {
    try {
      ...
      fetchUsers();  // Re-fetch users after approving
    } catch (error) {
      console.error("Error approving user:", error);
    }
  };

  const handleSetRole = async (userId: string, role: 'pending' | 'approved') => {
    try {
      const res = await fetch(`/api/users/${userId}/admin-role`, {
        method: 'POST',
  ...
  const handleLockUser = async (userId: string, currentStatus: string) => {
    try {
      ...
      fetchUsers();  // Re-fetch users after lock/unlock action
    } catch (error) {
  ...
  return (
    <div className="p-5 text-center">
      ...
      <h1 className="text-lg font-bold">Admin List</h1>
         ...
         <td className="py-3 px-4 text-center text-sm border-gray-300">
           ...
           {/* {user.status === 'approved' && ( */}
             <div className="relative inline-block">
               <select
                 onChange={(e) => handleSetRole(user.id, e.target.value as 'pending' | 'approved')}
                 defaultValue=""
               ...
               >
                 ...
                 <option className="bg-orange-500" value="pending">Unapproved</option>
                 <option className="bg-orange-500" value="approved">General</option>
               </select>
             </div>
           {/* )} */}
           {user.status !== 'pending' && (
